...
@Service
public class NotifyLowBalanceServiceAction extends AbstractActionNotificationEmailService {

    private final KafkaTemplate<String, Object> kafkaTemplate;

    public NotifyLowBalanceServiceAction(NotificationEmailSettingsRepository notificationEmailSettingsRepository,
    ...
    protected void applySetting(NotificationEmailSettings settings, boolean value) {
        settings.setNotifyOnWalletLowBalance(value);
    }

    @Override
    protected NotificationEmailDto mapToDto(NotificationEmailSettings settings) {
        return new NotificationEmailDto(settings.isNotifyOnWalletLowBalance(), null);
    }

    @Override
    protected void handleActivity(Long userId, boolean enabled) {
        kafkaTemplate.send("activity.settings", new SettingsActivityEvent(userId, SettingType.NOTIFICATION_WALLET_LOW_BALANCE, enabled ? SettingActivityStatus.ENABLED : SettingActivityStatus.DISABLED, LocalDateTime.now()));
    }
}
